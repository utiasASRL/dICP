...
def test_diff_inputs(source, target, max_iterations, tolerance):
    ...
    target_1 = torch.tensor(target[:55,:3], requires_grad=True)

    source_2 = torch.tensor(source[:,:3], requires_grad=True)
    target_2 = torch.tensor(target[:,:3], requires_grad=True)

    source_3 = torch.tensor(source[:55,:3], requires_grad=True)
    target_3 = torch.tensor(target[:60,:3], requires_grad=True)
    ...
    # Now pass as batch
    tic = time.time()
    source_transformed_batch, T_ts_pred_batch = pt2pt_dICP.icp(source_list, target_list, T_init_list, trim_dist=5.0, loss_fn=loss_fn, dim=2)
    toc = time.time()
